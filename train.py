...
# wasserstein loss for fake images
...
gen_train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(gen_cost, var_list=gen_params) # , tape=tf.GradientTape()
...
# Dataset iterator
def inf_train_gen():  # @@@ can also do it so it samples from another dataset every so often
    while True:
...
with tf.compat.v1.Session() as session:
    ...
    for iteration in range(args.iters + 1):
        ...
        # print("{}\t{}".format(iteration, np.mean(list(vals.values()))))
        print("iteration: " + str(iteration) + "    time: " + str(time.time() - start_time) + "    disc: " + str(_disc_cost))
